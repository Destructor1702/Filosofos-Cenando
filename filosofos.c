...
void eating(void *id) {
    ...
    printf("\nFilosofo %d: Comiendo",threadId+1);
}

void thinking(void *id) {
    int threadId = *(int *)id;
    think[threadId] += 1;
    printf("\nFilosofo %d: Pensando",threadId+1);
    sleep(5); // 5 segundos
}
...
void tryEating(void *id) {
    int threadId = *(int *)id;
    tried[threadId] += 1;
    printf("\nFilosofo %d: Tiene Hambre",threadId+1);
...
printf("\n Ejecucion  con %d filosofos y %d iteraciones \n",numthr,runningTime);
    ...
    sleep(runningTime);   //HILOS CORREN EN PARALELO AQUI
    runnig = 0;          //DETIENE LOS HILOS
